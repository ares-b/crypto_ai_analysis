...
from pipelines.raw.candles.models import BinanceCandleRow, RawBinanceCandle
from tests.conftest import make_kline

SETTINGS = BinanceCandleSettings(interval="1d")


class TestRawBinanceCandleFromApiResponse:
    def test_valid_payload(self):
        kline = RawBinanceCandle.from_api_response(make_kline())
    ...
    def test_wrong_length_raises(self):
        with pytest.raises(ValueError, match="length"):
            RawBinanceCandle.from_api_response(make_kline()[:11])

    def test_too_long_raises(self):
        with pytest.raises(ValueError, match="length"):
            RawBinanceCandle.from_api_response(make_kline() + ["extra"])

    def test_invalid_type_raises(self):
        bad = make_kline()
        bad[0] = "not_a_number"
        with pytest.raises(ValueError, match="Invalid"):
            RawBinanceCandle.from_api_response(bad)
